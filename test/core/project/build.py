...
def output_dir(config):
    return f"out/validation/{config.optimize}/{config.device[1]}"


def bl_output_dir(config):
    return f"out/bootloader/{config.optimize}/{config.device[1]}"
...
@matrix_command()
def cbuild(config):
    return ["cbuild", "--update-rte", "--configuration", f".{config.optimize}+{config.device[1]}", "validation.csolution.yml" ]
...
def model_exec(config):
    ...
    cmdline += ["-a", f"{build_dir(config)}/validation/{output_dir(config)}/{config.optimize}+{config.device[1]}.{config.compiler.image_ext}"]
    if config.device.has_bl():
        cmdline += ["-a", f"{build_dir(config)}/bootloader/{bl_output_dir(config)}/{config.optimize}+{config.device[1]}.{config.compiler.image_ext}"]
    return cmdline
