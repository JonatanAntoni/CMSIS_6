...
function generate() {
  ...
  mkdir -p "${DIRNAME}/../doc/$1/search/"
  cp -f "${DIRNAME}/template/search.css" "${DIRNAME}/../doc/$1/search/"

  sed -e "s/{datetime}/${datetime}/" "${DIRNAME}/template/footer.js.in" \
    | sed -e "s/{year}/${year}/" \
    | sed -e "s/{projectName}/${projectName}/" \
    | sed -e "s/{projectNumber}/${projectNumber}/" \
    | sed -e "s/{projectNumberFull}/${projectNumberFull}/" \
    > "${DIRNAME}/../doc/$1/footer.js"
